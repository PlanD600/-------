...
const TasksTab = ({ projects, teamMembers, refreshData, users }: TasksTabProps) => {
    ...
    const handleUpdateTask = async (updatedTaskData: Partial<TaskPayload>) => {
        if (!editingTask || !selectedProjectId) return;

        try {
            await api.updateTask(selectedProjectId, editingTask.task.id, updatedTaskData);
            const response = await api.getTasksForProject(selectedProjectId);
            setTasks(response.data);
            setEditingTask(null);
        } catch (error) {
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            setUserFilter('all');
                            localStorage.setItem('lastSelectedProjectId', newProjectId); // 💡 הוספנו את השורה הזו
                        }}

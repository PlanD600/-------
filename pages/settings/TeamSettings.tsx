...
const TeamSettings = ({ teams, users, allMemberships, refreshData }: TeamSettingsProps) => { 
    ...
    const potentialLeads = useMemo(() => {
        const leadRoles: Membership['role'][] = ['TEAM_LEADER', 'ADMIN', 'SUPER_ADMIN'];
        const leadUserIds = new Set(
            allMemberships.filter(m => leadRoles.includes(m.role)).map(m => m.user.id)
        );
        return users.filter(u => leadUserIds.has(u.id));

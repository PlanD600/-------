import React, { useMemo, Dispatch, SetStateAction } from 'react'; // <-- שינוי כאן: הוספת Dispatch ו-SetStateAction
...
    socket: Socket | null;
    refreshData: () => void;
    projectsView: 'active' | 'archived'; // פרופ חדש
    setProjectsView: Dispatch<SetStateAction<'active' | 'archived'>>; // פרופ חדש
}
...
const TabView = ({ activeTab, onTabChange, projects, teamMembers, teamLeads, users, teams, conversations, setConversations, socket, refreshData, projectsView, setProjectsView }: TabViewProps) => {
...
        { 
            id: 'overview', 
            label: 'מבט על', 
            component: ( // <-- שינוי כאן: העברת פרופסים projectsView ו-setProjectsView
                <OverviewTab 
                    projects={projects} 
                    teamLeads={teamLeads} 
                    users={users} 

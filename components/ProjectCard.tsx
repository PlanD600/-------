...
const ProjectCard = ({ project, onClick, onEdit, onArchive, onDelete }: ProjectCardProps) => {
    ...
    return (
        <div
            className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200 hover:shadow-md hover:border-gray-300 transition-all flex flex-col h-[192px] overflow-hidden"
        >
            <div className="flex-1 flex flex-col min-h-0">
                <div className="flex items-start gap-x-2 flex-none min-h-0">
                    <button
                        type="button"
                        className="flex-1 min-w-0 text-right space-y-1 p-2 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#4A2B2C] focus:ring-opacity-50 transition-colors flex flex-col"
                        onClick={onClick}
                    ...
                    <div className="w-40 flex-shrink-0 space-y-1 text-sm ml-1">
                        <DetailRow
                            label="סטטוס"
                            value={
